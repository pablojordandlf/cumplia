...
export default function DocumentsPage() {
  ...
  const loadDocuments = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/documents?organizationId=${membership.org_id}`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
  ...
  const loadDocumentTypes = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const types = Object.entries(data.types || {}).map(([type, info]: [string, any]) => ({
          type,
          title: info.title,
          ...
          upgrade_required: info.upgrade_required,
        }));
      ...
      setDocumentTypes([
        { type: 'ai_policy', title: 'Política de Uso de IA', description: 'Documento maestro de gobernanza de IA', requires_use_case: false, obligatory_for: ['all'], available: false, upgrade_required: true },
        { type: 'employee_notice', title: 'Aviso a Empleados', description: 'Información para empleados sobre sistemas de IA', requires_use_case: true, obligatory_for: ['high_risk'], available: false, upgrade_required: true },
        { type: 'systems_register', title: 'Registro de Sistemas', description: 'Inventario formal de sistemas de IA', requires_use_case: true, obligatory_for: ['high_risk'], available: false, upgrade_required: true },
        { type: 'fria', title: 'FRIA - Evaluación de Impacto', description: 'Evaluación de Impacto en Derechos Fundamentales', requires_use_case: true, obligatory_for: ['high_risk'], available: false, upgrade_required: true },
        { type: 'candidate_notice', title: 'Aviso a Candidatos', description: 'Información para candidatos en procesos de selección', requires_use_case: true, obligatory_for: ['high_risk', 'recruitment'], available: false, upgrade_required: true },
      ]);
    }
  };
  ...
  const handleGenerate = async (type: string) => {
    ...
    const isProRequired = schema?.minPlan === 'professional' && userPlan !== 'professional' ||
                          schema?.minPlan === 'essential' && userPlan === 'starter';
  ...
  const requiresPro = (type: string) => {
    const schema = documentSchemas[type];
    if (!schema) return userPlan === 'starter';
    
    if (schema.minPlan === 'professional') {
      return userPlan !== 'professional';
    }
    if (schema.minPlan === 'essential') {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {selectedDocType && (
        <DocumentGenerationWizard
          ...
          documentTitle={documentTypeLabels[selectedDocType]?.title || 'Documento'}
